...
import { 
  Mail, Phone, Award, BookOpen, GraduationCap, 
  Search, Users, MapPin, ExternalLink, Calendar
} from 'lucide-react';
...
const DepartmentFaculty: React.FC<DepartmentFacultyProps> = ({ departmentName, facultyData }) => {
      ...
      <CardContent className="p-6">
        <div className="flex flex-col sm:flex-row items-center sm:items-start space-y-4 sm:space-y-0 sm:space-x-4">
          ...
          <div className="flex-1 text-center sm:text-left">
            ...
            <p className="text-slate-600 mb-3 text-sm">{faculty.specialization}</p>
  ...
  const getTotalFaculty = () => {
    return Object.values(facultyData).reduce((total, group) => total + group.length, 0);
  };
    ...
    <section className="py-12 bg-gradient-to-br from-slate-50 to-white">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="text-center mb-12">
          ...
          <h2 className="text-3xl lg:text-4xl font-bold text-slate-900 mb-6">Our Expert Faculty Team</h2>
          <p className="text-lg text-slate-600 max-w-3xl mx-auto leading-relaxed">
            Meet our distinguished faculty members who bring decades of academic excellence and industry experience.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
          <Card className="text-center bg-white shadow-md border-0">
            <CardContent className="p-4">
              <div className="text-2xl font-bold text-blue-600">{getTotalFaculty()}</div>
              <div className="text-slate-600 text-sm">Total Faculty</div>
            </CardContent>
          </Card>
          <Card className="text-center bg-white shadow-md border-0">
            <CardContent className="p-4">
              <div className="text-2xl font-bold text-green-600">{facultyData.professors.length}</div>
              <div className="text-slate-600 text-sm">Professors</div>
            </CardContent>
          </Card>
          <Card className="text-center bg-white shadow-md border-0">
            <CardContent className="p-4">
              <div className="text-2xl font-bold text-purple-600">{facultyData.associateProfessors.length}</div>
              <div className="text-slate-600 text-sm">Associate Professors</div>
            </CardContent>
          </Card>
          <Card className="text-center bg-white shadow-md border-0">
            <CardContent className="p-4">
              <div className="text-2xl font-bold text-orange-600">{facultyData.assistantProfessors.length}</div>
              <div className="text-slate-600 text-sm">Assistant Professors</div>
            </CardContent>
          </Card>
          <Card className="text-center bg-white shadow-md border-0">
            <CardContent className="p-4">
              <div className="text-2xl font-bold text-red-600">{facultyData.guestFaculty.length}</div>
        ...
        <Tabs defaultValue="all" className="w-full">
          <TabsList className="grid grid-cols-3 lg:grid-cols-6 w-full lg:w-fit mx-auto mb-8 h-auto p-1 bg-white shadow-lg rounded-xl">
            <TabsTrigger value="all" className="px-3 py-2 rounded-lg text-sm">All Faculty</TabsTrigger>
            <TabsTrigger value="coordinator" className="px-3 py-2 rounded-lg text-sm">Coordinator</TabsTrigger>
            <TabsTrigger value="professors" className="px-3 py-2 rounded-lg text-sm">Professors</TabsTrigger>
            <TabsTrigger value="associate" className="px-3 py-2 rounded-lg text-sm">Associate Prof.</TabsTrigger>
            <TabsTrigger value="assistant" className="px-3 py-2 rounded-lg text-sm">Assistant Prof.</TabsTrigger>
            <TabsTrigger value="guest" className="px-3 py-2 rounded-lg text-sm">Guest Faculty</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="all">
            <div className="space-y-8">
              ...
              {facultyData.coordinator.length > 0 && (
                <div>
                  <h3 className="text-xl font-bold text-slate-900 mb-4">Department Coordinator</h3>
              ...
              {facultyData.professors.length > 0 && (
                <div>
                  <h3 className="text-xl font-bold text-slate-900 mb-4">Professors</h3>
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              {facultyData.associateProfessors.length > 0 && (
                <div>
                  <h3 className="text-xl font-bold text-slate-900 mb-4">Associate Professors</h3>
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {filterFaculty(facultyData.associateProfessors).map((faculty, index) => (
                      <FacultyCard key={index} faculty={faculty} />
                    ))}
                  </div>
                </div>
              ...
              {facultyData.assistantProfessors.length > 0 && (
                <div>
                  <h3 className="text-xl font-bold text-slate-900 mb-4">Assistant Professors</h3>
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {filterFaculty(facultyData.assistantProfessors).map((faculty, index) => (
                      <FacultyCard key={index} faculty={faculty} />
                    ))}
                  </div>
                </div>
              ...
              {facultyData.guestFaculty.length > 0 && (
                <div>
                  <h3 className="text-xl font-bold text-slate-900 mb-4">Guest Faculty</h3>
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
